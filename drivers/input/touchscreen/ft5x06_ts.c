...
#define FT5x0x_RST	IMX_GPIO_NR(7, 11) 
#define FT5x0x_INT	IMX_GPIO_NR(3, 25) 
...
//    .irqflags = IRQF_TRIGGER_RISING,// CR
	...
	.irq_gpio  = FT5x0x_INT,//133 on CR,142 for RVP
	.reset = FT5x0x_RST,//128 on CR, 60 on RVP
};
...
static int ft5x0x_read_Touchdata(struct ft5x0x_ts_data *data)
{
	...
	//pr_info("~~~~~~~~ Into FT5x0x Read Touchdata ~~~~~~~~~\n");
			   ...
			   const struct i2c_device_id *id)
{
	...
	err = request_threaded_irq(client->irq, NULL, ft5x0x_ts_interrupt, pdata->irqflags | IRQF_ONESHOT, client->dev.driver->name, ft5x0x_ts);
	...
	ft5x0x_i2c_Write(client, &uc_reg_addr, 1);
	ft5x0x_i2c_Read(client, &uc_reg_addr, 0, &uc_reg_value, 1);
	...
	uc_reg_addr = FT5x0x_REG_CHIP_ID;
	ft5x0x_i2c_Write(client, &uc_reg_addr, 1);
	ft5x0x_i2c_Read(client, &uc_reg_addr, 0, &uc_reg_value, 1);
	pr_info("[FTS] chip id is %d.\n",uc_reg_value);
